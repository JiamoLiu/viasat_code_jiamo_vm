...
#NETFACE=enp0s3 # interface which in connected to the internet
NETFACE="delay-516890"
...
function createLimit {
  ...
  #tc qdisc add dev $NETFACE root tbf rate "$LIMIT"mbit latency 100ms burst 10000
  #tc qdisc add dev $IFACE root tbf rate "$LIMIT"mbit latency 100ms burst 10000
  
   
  
  #tc qdisc add dev $NETFACE root htb rate "$LIMIT"mbit ceil "$LIMIT"mbit 
  #tc qdisc add dev $IFACE root htb rate "$LIMIT"mbit ceil "$LIMIT"mbit

  #tc qdisc add dev $NETFACE root bfifo limit "$LIMIT"800
  #tc qdisc add dev $IFACE root bfifo limit "$LIMIT"800
}
function updateLimit {
  ...
  #sudo tc qdisc change dev $NETFACE root tbf rate "$LIMIT"mbit latency 100ms burst 10000
  #sudo tc qdisc change dev $IFACE root tbf rate "$LIMIT"mbit latency 100ms burst 10000
 
  #sudo tc qdisc change dev $NETFACE root htb rate "$LIMIT"mbit ceil "$LIMIT"mbit
  #sudo tc qdisc change dev $IFACE root htb rate "$LIMIT"mbit ceil "$LIMIT"mbit
