...
async function onYouTubePlayerReady(playerId) {
...
//			console.log("YouSlow: fraction - "+fraction);
	...
	//Get bufferhealth
	setInterval(function(){
		bufferhealth = bufferhealth + " " +player.getVideoLoadedFraction().toString();
		playback_times = playback_times + " " +player.getCurrentTime().toString();
		console.log("my buffer health is:" + player.getVideoLoadedFraction().toString());
		console.log("my concat bufferhealth is:" + bufferhealth)
	},100);
...
//		console.log(videoduration_one_second_ago+" : "+videoduration_two_second_ago);
	...
	if(isGood || !isGood){
	    ...
	    var videoInfoURL = report_ep+(URLparameters)
...
function report(){
	...
	if(isGood || !isGood){
		...
		var xhr = new XMLHttpRequest();
		xhr.open("POST", videoInfoURL, true);
		...
		var data = `{"bufferFraction": "${bufferhealth}","playbackSeries": "${playback_times}"}`;
		xhr.send(data);
